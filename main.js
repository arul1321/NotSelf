...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    zeroyt7.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Subscribe ArulGanz')
    })
...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya AbilBotz saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk meulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        but = [
          { buttonId: 'add', buttonText: { displayText: 'Welcome Jangan Baperan & Jangan Jadi Beban' }, type: 1 }
        ]
        sendButImage(mdata.id, wel, "©Created : ArulBotz", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Created : ArulBotz", buff, but)
      }
